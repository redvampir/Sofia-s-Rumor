...
- Формат тела ошибки (application/json):
  ...
  ```
  - `code` — машинно-читаемый код (например, `VALIDATION_FAILED`, `RATE_LIMITED`, `INTERNAL_ERROR`).
...
- Сетевые тайм-ауты: подключение до 5 секунд, чтение тела запроса/ответа до 60 секунд; по истечении сервер возвращает `500` с кодом `TIMEOUT`.
